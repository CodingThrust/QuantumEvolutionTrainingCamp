function YaoAPI.measure(
    ...
    return DitStr64{D, reg.subspace.n}.(YaoArrayRegister.sample(rng, reg.subspace.basis, Weights(abs2.(relaxedvec(reg))), nshots))
end
...
function YaoAPI.measure!(
    ...
    ind = YaoArrayRegister.sample(rng, 1:length(reg.subspace.basis), Weights(abs2.(relaxedvec(reg))))
    fill!(reg.state, 0)
    reg.state[ind] = 1
    return DitStr64{D,reg.subspace.n}(vec(reg.subspace.basis)[ind])
end
