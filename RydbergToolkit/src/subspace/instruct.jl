function YaoAPI.unsafe_apply!(reg::SubspaceArrayReg{D, T}, te::TimeEvolution) where {D, T}
    H = mat(T, te.H, reg.subspace)
    if H isa Diagonal
        # `compatible_multiplicative_operand` is used to ensure GPU compatibility.
        reg.state .*= exp.((-im * te.dt) .* YaoBlocks.compatible_multiplicative_operand(reg.state, H.diag))
        return reg
    end
    dt = real(te.dt) == 0 ? imag(te.dt) : -im * te.dt
    v_out, info = YaoBlocks.exponentiate(H, dt, reg.state,
                                tol=te.tol,
...
function YaoAPI.instruct!(r::SubspaceArrayReg{D}, op, locs::Tuple, control_locs::Tuple, control_bits::Tuple) where D
    ...
    _instruct!(res, r.state, r.subspace.basis, mp, YaoArrayRegister.staticize(op), locs, mask, onemask)
    r.state .= res
    return r
end
...
function YaoAPI.instruct!(r::SubspaceArrayReg{D}, op, locs::Tuple, ::Tuple{}, ::Tuple{}) where D
    ...
    res = zero(r.state)
    mp = r.subspace.map   # this is slower than regular dict!
    _instruct!(res, r.state, r.subspace.basis, mp, YaoArrayRegister.staticize(op), locs)
    r.state .= res
    return r
end

for OP in YaoArrayRegister.SPECIALIZATION_LIST
    @eval _mat(::Type{T}, ::Val{$(QuoteNode(OP))}) where {T} = mat(T, ConstGate.$OP)
end
...
function YaoAPI.instruct!(r::SubspaceArrayReg, op, locs::Tuple, theta::Number)
    return instruct!(r, op, locs, (), (), theta)
end

function _instruct!(res, state, subspace_basis, map, m, locs)
    locs_zeroclear_mask = ~bmask(locs...)
    @inbounds for j in 1:length(subspace_basis)
        J = subspace_basis[j]
...
function _instruct!(res, state, subspace_basis, map, m, locs, mask, onemask)
    locs_zeroclear_mask = ~bmask(locs...)
    @inbounds for j in 1:length(subspace_basis)
        J = subspace_basis[j]
